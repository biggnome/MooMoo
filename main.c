/* main.c - MooMoo, a stream encoder/decoder */
...
int main(int argc, char *argv[])
{
  ...
  if ((codec_op = pick_operation(argv[0])) == MM_OP_INVALID) {
    errno = EINVAL;
    perror("moo/unmoo argv[0] unrecognized!");
    exit(-1);
    /* NOTREACHED */
  }
...
int pick_operation(char *argv0)
{
  ...
  if (strncmp(name, CMD_MOO, strlen(CMD_MOO)) == 0)
    return MM_OP_ENCODE;

  if (strncmp(name, CMD_UNMOO, strlen(CMD_UNMOO)) == 0)
    return MM_OP_DECODE;
